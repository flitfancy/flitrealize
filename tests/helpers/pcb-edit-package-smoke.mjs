...
try {
  ...
  await writeFile(inputFile, JSON.stringify({ ...target, rules: [{ net: 'PWR', primitiveIds: ['l1'], targetWidthMil: 25 }] }));
  const options = { projectRoot, inputFile, action: 'pcb-trace-width', invoke };
  ...
  assert.equal(failed.status, 'save-failed'); assert.equal(scene.writes.length, 1);
  ...
  assert.equal(scene.writes.length, 1); assert.equal(scene.saves, 1);
